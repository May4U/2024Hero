...
//    Robot_cmd.RC->mouse.press_l = DJI_Motor_Rx_Data[2];
//    Robot_cmd.RC->mouse.press_r = DJI_Motor_Rx_Data[3];
}
...
/**
  ...
  *@note  字长4字节
  */
uint8_t gimbal_fire_ready = 0;
uint8_t gimbal_open_auto = 0;//云台开启自瞄
uint8_t Get_fire_ready(void){return gimbal_fire_ready;}
uint8_t Get_gimbal_open_auto(void){return gimbal_open_auto;}
...
{    
    ...
    gimbal_fire_ready = (Rx_Data[0]>>2) & 1;//最低3位，摩擦轮开启？
    gimbal_open_auto = (Rx_Data[0]>>3) & 1;//最低4位，手打？
...
//    if (Robot_cmd.RC->rc.s2 == RC_SW_DOWN){//遥控器左侧开关状态为[下]
...
static void NO_FOLLOW_Mode()
{
	Robot_cmd.Speed_set.chassis_z = 0;
    #ifdef USE_NAVIGATION
    Robot_cmd.Speed_set.chassis_z = (-RC_ctrl->rc.ch[2] * 0.1f);
    #endif
...
{
    Z_close = Robot_cmd.Difference_Angle_between_Chassis_Gimbal;
    if (Z_close > 90)
    {
        Z_close = -180 + Z_close;
    }
...
#ifdef USE_NAVIGATION
extern Navi_Speed_t Navi_Speed;
static void NAVIGATION_Mode()
{
    Robot_cmd.Speed_set.chassis_x = Navi_Speed.x * 2.0f;
    Robot_cmd.Speed_set.chassis_y = -Navi_Speed.y * 2.0f;
    Robot_cmd.Speed_set.chassis_z = Navi_Speed.z * 1.25f;
}
#endif
/**
...
//    */
//    
    //Robot_cmd.Speed_set.chassis_z = -(Robot_cmd.RC->rc.ch[0] ) / 10.0f;
...
//    {
//        //摇杆速度死区限制
//        if ( ABS_FP(Robot_cmd.Speed_set.chassis_x) > SPEED_DEADBAND || ABS_FP(Robot_cmd.Speed_set.chassis_y) > SPEED_DEADBAND )
//            return;
//        //陀螺仪加速度检测，这样做防止高速下遥控突然回正导致直接刹车锁死
//        if( ABS_FP(ins->E_Accel[X]) >= 0.1f || ABS_FP(ins->E_Accel[Y]) >= 0.1f) 
//            return;
//    }      
}
...
void Fire_Set(void)
{
    ...
    if (Robot_cmd.RC->rc.ch[4] <200 && Robot_cmd.RC->mouse.press_l == 0) 
        rc_4_reset = true;//回正检测
    if (gimbal_fire_ready == 0)                 Robot_cmd.Fire_State = NO_Fire_FORCE;//关摩擦轮
    else if(Robot_cmd.Fire_State != On_Fire)    Robot_cmd.Fire_State = Ready_Fire;   //开摩擦轮
    
    if (Robot_cmd.RC->rc.ch[4] <-200 || Robot_cmd.RC->kb.bit.M == 1)   Robot_cmd.Fire_State = On_Empty;//退蛋
//    static Fire_State_e Last_RC_Fire_State = NO_Fire_FORCE;
//    static Fire_State_e RC_Fire_State   = NO_Fire_FORCE;

//    switch(Robot_cmd.RC->rc.s2){//遥控右侧开关
//        case RC_SW_DOWN://开关状态为[下]
//            RC_Fire_State = Ready_Fire;//关摩擦轮
//            break;
//        case RC_SW_MID://开关状态为[中]
//            RC_Fire_State = Ready_Fire;//开摩擦轮
//            break;
...
//            break;
//        }
//    if (Last_RC_Fire_State != RC_Fire_State)
//    {
//        Robot_cmd.Fire_State = RC_Fire_State;
//    }
//    Last_RC_Fire_State = RC_Fire_State;
//    if (Robot_cmd.Fire_State == On_Empty)
//        Robot_cmd.Fire_State = RC_Fire_State;


//    if(Robot_cmd.RC->kb.bit.F == 1)  
//        Robot_cmd.Fire_State = Ready_Fire;
    
    if (Robot_cmd.Fire_State == NO_Fire_FORCE || Robot_cmd.Fire_State == On_Empty)  return;
        
    if (gimbal_open_auto == 1)//自瞄
    {
...
//    if (Robot_cmd.RC->rc.s2 == RC_SW_UP || Robot_cmd.RC->mouse.press_r == 1)//自瞄火控
//    {
//        if ( (Robot_cmd.RC->rc.ch[4] >= 550 || Robot_cmd.RC->mouse.press_l == 1) && rc_4_reset && Robot_cmd.AUTO_Fire_Flag == 1)
//        {
//            Robot_cmd.Fire_State = On_Fire;
//            rc_4_reset = false;
//            Robot_cmd.increat_time ++;
...
//    }
//    
//    if (Robot_cmd.RC->rc.s2 == RC_SW_MID)//手动火控
//    {
//        if ( (Robot_cmd.RC->rc.ch[4] >= 550 || Robot_cmd.RC->mouse.press_l == 1) && rc_4_reset)
//        {
//            Robot_cmd.Fire_State = On_Fire;
