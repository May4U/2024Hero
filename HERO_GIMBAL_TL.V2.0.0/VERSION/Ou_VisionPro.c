...
{       
    ...
    {       
        //转速阈值化
        if (auto_control_p->v_yaw > 1.0f)           auto_control_p->v_yaw = 2.51327408;//PI*0.8
        else if (auto_control_p->v_yaw < -1.0f)     auto_control_p->v_yaw = -2.51327408;
        else                                        auto_control_p->v_yaw = 0;
        
        auto_control_p->yaw += auto_control_p->v_yaw * timeDelay;//计算yaw轴需要旋转的角度

         for (i = 0; i<3; i++) {
            float tmp_yaw = auto_control_p->yaw + i * 2.0 * PI/3.0;  // 2/3PI
            float r =  0.2765;   //理论上r1=r2 这里取个平均值
            tar_position[i].x = auto_control_p->xw - r*cos(tmp_yaw);
            tar_position[i].y = auto_control_p->yw - r*sin(tmp_yaw);
            tar_position[i].z = auto_control_p->zw;
        ...
        fp32 YawAngleToRotaCenter = (float)(atan2(auto_control_p->yw,auto_control_p->xw)*180.0f/PI);//把设定的yaw轴转换到imu设定值
...
//        }
//               
//        uint8_t Shake_Yaw_Fire = 0;//强制指定摇头打法
//        uint8_t Shake_Pitch_Fire = 1;
//        if (Shake_Yaw_Fire == 1)//摇头打法
//        {
//            
//            tar_position[0].angle_yaw = auto_control_p->yaw * 180.f / PI;
//            //tar_position[0].angle_yaw -= YawAngleToRotaCenter;
...
//            for(int i = 1; i < 4; i++)
//            {
//                if (tar_position[i].distance < min_distance -0.08)
//                {
//                    min_distance = tar_position[i].distance;
//                    min_distance_idx = i;
//                }
...
//            auto_control_p->Target_Position.distance = sqrtf(auto_control_p->Target_Position.x * auto_control_p->Target_Position.x + auto_control_p->Target_Position.y * auto_control_p->Target_Position.y);
///**************************************转换到pitch/yaw设定角度*************************************/
//            auto_control_p->auto_pitch = (float)(pitchTrajectoryCompensation(auto_control_p->Target_Position.distance,auto_control_p->Target_Position.z,auto_control_p->bullet_speed )* 180.f / PI) + auto_control_p->pitch_compensate;
//            auto_control_p->auto_yaw = (float)((atan2(auto_control_p->Target_Position.y,auto_control_p->Target_Position.x)*180.0f/PI));//把设定的yaw轴转换到imu设定值//右- 左+
// 
//            //判断开火条件
//            if (tar_yaw_close < max_yaw_close && (abs(auto_control_p->auto_yaw - auto_control_p->Imu_c->Yaw) < max_yaw_close) && (abs(auto_control_p->auto_pitch - auto_control_p->Imu_c->Pitch) < max_pitch_close)) 
//                auto_control_p->fire_flag = 1;
//            else  
//                auto_control_p->fire_flag = 0;
//                
//        }
//        else if (Shake_Yaw_Fire == 0)//定头打法
//        {
//            float close_r = 0;
...
//                }
//            }
//            fp32 Rota_center_x = auto_control_p->xw;
//            fp32 Rota_center_y = auto_control_p->yw;
//            //旋转中心-面前板到旋转中心半径，得到当 面前板旋转到 枪口与旋转中心连线时的水平距离
//            fp32 distance = sqrtf(Rota_center_x*Rota_center_x + Rota_center_y*Rota_center_y) - close_r;
//            fp32 pitch_hight;
//            fp32 min_hight;
//            
//            if (Shake_Pitch_Fire == 0)
//            {//不动Pitch,固定锁某个装甲板高度
//                min_hight = min((tar_position[0].z),(tar_position[1].z));
//                pitch_hight = min((tar_position[0].z),(tar_position[1].z));
//            }
//            else
//            {//动Pitch
//                pitch_hight = tar_position[idx].z;
//            }
//            
//            
//            //auto_control_p->auto_pitch = completeAirResistanceModel(auto_control_p, distance, pitch_hight) + auto_control_p->pitch_compensate;
//            auto_control_p->auto_pitch = (float)(pitchTrajectoryCompensation(distance, pitch_hight, auto_control_p->bullet_speed )* 180.f / PI) + auto_control_p->pitch_compensate;
...
//                if (abs(tar_yaw[i] - auto_control_p->auto_yaw) < max_yaw_close)
//                {
//                    if (abs(auto_control_p->auto_pitch - auto_control_p->Imu_c->Pitch) < max_pitch_close)
//                    {
//                        fire_control = 1;
//                        break;
//                    }
...
//            auto_control_p->Target_Position.z = tar_position[idx].z;
//            auto_control_p->Target_Position.x = tar_position[idx].x;
//            auto_control_p->Target_Position.y = tar_position[idx].y;	
//            auto_control_p->Target_Position.distance = tar_position[idx].distance;
//            
//            auto_control_p->auto_pitch = (float)(pitchTrajectoryCompensation(auto_control_p->Target_Position.distance,auto_control_p->Target_Position.z,auto_control_p->bullet_speed )* 180.f / PI) + auto_control_p->pitch_compensate;
//            auto_control_p->auto_yaw = (float)((atan2(auto_control_p->Target_Position.y,auto_control_p->Target_Position.x)*180.0f/PI));//把设定的yaw轴转换到imu设定值//右- 左+
//        }
