...
static void Max_Speed_Limit(void);
static void chassis_prevent_motion_distortion(void);
...
static void Chassis_Motor_Set_Val(void);
...
//                True_power_limit(160);

                        True_power_limit(Cap_Input_Power);
...
void Chassis_Init(void)
{
    ...
    Chassis.RF_MOTOR = DJIMotor_Init(1,3,false,M3508,178);
    Chassis.RB_MOTOR = DJIMotor_Init(1,2,false,M3508,178);
    Chassis.LB_MOTOR = DJIMotor_Init(1,1,true,M3508,178);
    Chassis.LF_MOTOR = DJIMotor_Init(1,4,true,M3508,178);
    ...
    pm01_power_set((50)*100-50,0x0001);//50w
}
/**
  *@brif 底盘驱动电机设置PID环
  */
void Chassis_Motor_Set_PID(void)
{
	...
	{
		Chassis.RF_MOTOR->Using_PID = No_Current;
        Chassis.RB_MOTOR->Using_PID = No_Current;
        Chassis.LB_MOTOR->Using_PID = No_Current;
        Chassis.LF_MOTOR->Using_PID = No_Current;
	}
...
/**
  *@brif 底盘电机PID外环设置目标值
  */
void Chassis_Motor_Set_Val(void)
{
static uint8_t first_set_lock_angle = 1;
    if (Chassis.Robot_cmd->Chassis_State != LOCK_POSITION)
    {
        first_set_lock_angle = 1;
    }
        
    if(Chassis.Robot_cmd->Chassis_State == LOCK_POSITION)
	{
        if (first_set_lock_angle == 1)
        {
            if (abs(Chassis.LF_MOTOR->Motor_Information.speed < 10) && abs(Chassis.RF_MOTOR->Motor_Information.speed < 10) && abs(Chassis.LB_MOTOR->Motor_Information.speed < 10) && abs(Chassis.RB_MOTOR->Motor_Information.speed < 10))
            {
                Chassis.RF_MOTOR->Using_PID = Position_Speed_PID;
                Chassis.RB_MOTOR->Using_PID = Position_Speed_PID;
                Chassis.LB_MOTOR->Using_PID = Position_Speed_PID;
...
void CAN_Send_3508_5_8_current( int16_t LF_Motor, int16_t RF_Motor, int16_t LB_Motor, int16_t RB_Motor)
{
...
/**
  *@brif 防止底盘运动失真
  *@note True_power_limit已可以保证运动不失真，放弃使用此函数
  */
void chassis_prevent_motion_distortion(void)
{
	float divisor = 1.0f;
    uint64_t ALL_motor_current = ABS(Chassis.LB_MOTOR->Speed_PID.out) + ABS(Chassis.LF_MOTOR->Speed_PID.out) + ABS(Chassis.RB_MOTOR->Speed_PID.out) + ABS(Chassis.RF_MOTOR->Speed_PID.out);
    if (ALL_motor_current > 25000.0f)
    {
		divisor = 25000.0f/ALL_motor_current;
	}
    Chassis.LF_MOTOR->current_input = Chassis.LF_MOTOR->Speed_PID.out*divisor;
    Chassis.RF_MOTOR->current_input = Chassis.RF_MOTOR->Speed_PID.out*divisor;
    Chassis.LB_MOTOR->current_input = Chassis.LB_MOTOR->Speed_PID.out*divisor;
    Chassis.RB_MOTOR->current_input = Chassis.RB_MOTOR->Speed_PID.out*divisor;
}
/**
  *@brif 根据超电容量设置最大速度
  *@note 考虑按键主动开关
  */
void Max_Speed_Limit(void)
{
    float divisor;
    float superPower_v;
    
    superPower_v = ((float) Chassis.pm01_od->v_out) / 100.0f;
    divisor = ((float)(superPower_v/24.0f)) * ((float)(superPower_v/24.0f));
    
    if (divisor <= 0.3f)
    {
        divisor = 0.3f;
    }
    Chassis.Robot_cmd->Speed_set.LB_motor *= divisor;
    Chassis.Robot_cmd->Speed_set.LF_motor *= divisor;
    Chassis.Robot_cmd->Speed_set.RB_motor *= divisor;
    Chassis.Robot_cmd->Speed_set.RF_motor *= divisor;
}
/**
  *@brif 输出电流线性缩放
  *@note 无法准确控制真实功率
...
{	
	...
	float    Joule_Residue = 0;//超电电压

	static int32_t judgDataError_Time = 0;
	float fTotalCurrentLimit;

	Joule_Residue = Chassis.pm01_od->v_out / 100;//超电电压

	//统计底盘总输出
	chassis_totaloutput = ABS(	Chassis.LB_MOTOR->Speed_PID.out) + ABS(	Chassis.LB_MOTOR->Speed_PID.out)
							+ ABS(	Chassis.LB_MOTOR->Speed_PID.out) + ABS(	Chassis.LB_MOTOR->Speed_PID.out);
	
	if(Joule_Residue <= 13)//低压区间
	{
        fTotalCurrentLimit = 8000;//降为最大的1/4 - 1000
	}
	else if (Joule_Residue >= 20)//释放区间
    {
        fTotalCurrentLimit = MOTOR_3508_CURRENT_LIMIT*4;
    }
	else//缩放区间
    {
		//剩余焦耳量过小,开始限制输出,限制系数为4次方关系
        kLimit = (float)(Joule_Residue / 20)
               * (float)(Joule_Residue / 20);
        
        fTotalCurrentLimit = kLimit * MOTOR_3508_CURRENT_LIMIT*4;
	}

	//底盘各电机电流重新分配
	if (chassis_totaloutput > fTotalCurrentLimit)
	{
		Chassis.LB_MOTOR->Speed_PID.out = (int16_t)(Chassis.LB_MOTOR->Speed_PID.out / chassis_totaloutput * fTotalCurrentLimit);
		Chassis.RB_MOTOR->Speed_PID.out = (int16_t)(Chassis.RB_MOTOR->Speed_PID.out / chassis_totaloutput * fTotalCurrentLimit);
		Chassis.LF_MOTOR->Speed_PID.out = (int16_t)(Chassis.LF_MOTOR->Speed_PID.out / chassis_totaloutput * fTotalCurrentLimit);
		Chassis.RF_MOTOR->Speed_PID.out = (int16_t)(Chassis.RF_MOTOR->Speed_PID.out / chassis_totaloutput * fTotalCurrentLimit);	
	}
}
/**
  *@brif 准确限制输入功率，输出电流非线性缩放
  */
void True_power_limit(uint8_t Chassis_Power_Limit)
{
    ...
    //P_in[RF] = Chassis.RF_MOTOR->Speed_PID.out*K*Chassis.RF_MOTOR->Motor_Information.speed + K_1*Chassis.RF_MOTOR->Motor_Information.speed*Chassis.RF_MOTOR->Motor_Information.speed + a*Chassis.RF_MOTOR->Speed_PID.out*Chassis.RF_MOTOR->Speed_PID.out;
    //P_in[RB] = Chassis.RB_MOTOR->Speed_PID.out*K*Chassis.RB_MOTOR->Motor_Information.speed + K_1*Chassis.RB_MOTOR->Motor_Information.speed*Chassis.RB_MOTOR->Motor_Information.speed + a*Chassis.RB_MOTOR->Speed_PID.out*Chassis.RB_MOTOR->Speed_PID.out;
    //P_in[LF] = Chassis.LF_MOTOR->Speed_PID.out*K*Chassis.LF_MOTOR->Motor_Information.speed + K_1*Chassis.LF_MOTOR->Motor_Information.speed*Chassis.LF_MOTOR->Motor_Information.speed + a*Chassis.LF_MOTOR->Speed_PID.out*Chassis.LF_MOTOR->Speed_PID.out;
    //P_in[LB] = Chassis.LB_MOTOR->Speed_PID.out*K*Chassis.LB_MOTOR->Motor_Information.speed + K_1*Chassis.LB_MOTOR->Motor_Information.speed*Chassis.LB_MOTOR->Motor_Information.speed + a*Chassis.LB_MOTOR->Speed_PID.out*Chassis.LB_MOTOR->Speed_PID.out;
    /*---底盘输出功率总和---*/
    Chassis_Power_All = 0;
    for(int i = RF;i <= LF; i++)
    {
...
uint8_t Calc_SuperPower_Out_power_limit(void)
{
        ...
        if (Chassis.pm01_od->energy >= 30)   return 120;//160w功率
